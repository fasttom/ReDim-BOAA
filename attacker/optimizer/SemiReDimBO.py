...
from optimizer.utils import perturbate_semi, relative_loss_gain, random_noise, image_interpolarate
from botorch.acquisition import ExpectedImprovement, UpperConfidenceBound
from botorch.optim import optimize_acqf

def SemiReDimBO(image:torch.Tensor,
    ...
    encoded_shape = autoencoder.encoder(image.unsqueeze(0)).squeeze(0).shape
    deltas = [random_noise(encoded_shape).flatten() for _ in range(20)] #initial 20 random perturbations
    z = autoencoder.encoder(image.unsqueeze(0)).squeeze(0)
    perturbated_zs = [perturbate_semi(z, delta) for delta in deltas]
    ...
    for i in range(epoch_lim-20):
        ...
        perturbated_z = perturbate_semi(z, candidate)
